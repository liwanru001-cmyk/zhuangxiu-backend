...
function createFieldReview(db){
  ...
  return {
    ...
    async saveSelections(id,body,actor){
      ...
      await db.query(`UPDATE product_ingestion_candidates SET normalized_payload=?,generated_fields=?,review_status='pending',review_note=NULL,reviewed_by=NULL,reviewed_at=NULL,manual_revision=manual_revision+1 WHERE id=?`,[JSON.stringify(payload),JSON.stringify(generated),candidate.id]);
    ...
    async applyFieldMapping(id,body,actor){
      ...
      for(const match of matches){
        ...
        await db.query(`UPDATE product_ingestion_candidates SET normalized_payload=?,generated_fields=?,review_status='pending',review_note=NULL,reviewed_by=NULL,reviewed_at=NULL,manual_revision=manual_revision+1 WHERE id=? AND published_product_id IS NULL`,[JSON.stringify(match.payload),JSON.stringify(generated),match.row.id]);
      }
      let savedRule=false;
