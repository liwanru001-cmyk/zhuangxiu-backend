...
const PROMPT_VERSION = 'ai-design-v2.1';
...
async function request({ source, settings, manifest, repair, limits, signal, reserve, record, fetchImpl = fetch, env = process.env }) {
  ...
  const content = [{ type: 'text', text: JSON.stringify({
    ...
    execution: { supported_elements: ['text', 'image', 'shape', 'line'], total_elements_max: 600,
      ...
      boundaries: '所有元素旋转后仍须在页面内，文字完整可见。避免遮挡正文；可自主设计图上标题、遮罩和装饰。',
    },
  ...
  for (const asset of manifest.filter(a => a.vision_preview_provided && (!needed || needed.has(a.asset_id)))) {
    content.push({ type: 'text', text: `以下代表图对应 asset_id=${asset.asset_id}` });
