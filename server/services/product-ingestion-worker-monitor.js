...
function createWorkerMonitor(db, env = process.env) {
  ...
  async function collect(){
    const [[workers],[restartRows],[jobs]]=await Promise.all([
    ...
    const worker=workers[0]||null,alerts=[];
    if(!worker||worker.status!=='online'||Number(worker.heartbeat_age_seconds)>staleSeconds) alerts.push({key:`worker-heartbeat:${instanceId}`,type:'WORKER_HEARTBEAT_OR_DB_TUNNEL_STALE',severity:'critical',instance_id:instanceId,message:'抓取 Worker 心跳失联；Worker、PM2 或 SSH 数据库隧道可能不可用。',details:{worker}});
