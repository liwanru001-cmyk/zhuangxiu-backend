  ...
  return {
    ...
    async listCandidates(query = {}) {
      ...
      const selectableCategoryCountSql=`CASE
        WHEN EXISTS (SELECT 1 FROM product_ingestion_candidate_categories manual_assignment WHERE manual_assignment.candidate_id=candidate.id AND manual_assignment.assignment_type='manual') THEN (SELECT COUNT(*) FROM product_ingestion_candidate_categories assignment JOIN public_product_categories category ON category.id=assignment.category_id WHERE assignment.candidate_id=candidate.id AND assignment.assignment_type='manual' AND category.status='active' AND NOT EXISTS (SELECT 1 FROM public_product_categories child WHERE child.parent_id=category.id AND child.status='active'))
        WHEN EXISTS (SELECT 1 FROM product_ingestion_discovered_product_categories evidence JOIN product_ingestion_category_mappings mapping ON mapping.source_category_id=evidence.source_category_id WHERE evidence.job_id=candidate.job_id AND evidence.product_url_hash=candidate.source_url_hash) THEN (SELECT COUNT(DISTINCT mapping.category_id) FROM product_ingestion_discovered_product_categories evidence JOIN product_ingestion_category_mappings mapping ON mapping.source_category_id=evidence.source_category_id JOIN public_product_categories category ON category.id=mapping.category_id WHERE evidence.job_id=candidate.job_id AND evidence.product_url_hash=candidate.source_url_hash AND category.status='active' AND NOT EXISTS (SELECT 1 FROM public_product_categories child WHERE child.parent_id=category.id AND child.status='active'))
        ELSE (SELECT COUNT(*) FROM product_ingestion_candidate_categories assignment JOIN public_product_categories category ON category.id=assignment.category_id WHERE assignment.candidate_id=candidate.id AND assignment.assignment_type='system' AND category.status='active' AND NOT EXISTS (SELECT 1 FROM public_product_categories child WHERE child.parent_id=category.id AND child.status='active'))
      END`;
      const params = []; let where = 'WHERE job.deleted_at IS NULL';
      ...
      let total=null;
      if(paged){const [countRows]=await db.query(`SELECT COUNT(*) total FROM product_ingestion_candidates candidate JOIN product_ingestion_sources source ON source.id=candidate.source_id JOIN product_ingestion_jobs job ON job.id=candidate.job_id ${where}`,params);total=Number(countRows[0]?.total||0);}
      const [rows] = await db.query(`SELECT candidate.id,candidate.job_id,candidate.source_id,candidate.source_url,candidate.source_external_id,candidate.normalized_payload,candidate.validation_status,candidate.validation_issues,candidate.review_status,candidate.generated_fields,candidate.classification_suggestion,candidate.classification_override,candidate.published_product_id,candidate.published_version_id,candidate.published_at,candidate.created_at,source.brand_name,source.status source_status,job.status job_status,job.current_stage,
        ...
        ${selectableCategoryCountSql} selectable_category_count,
      ...
      const items=rows.map(row => {const payload=parseJsonValue(row.normalized_payload),summary=candidateSummary(payload),categoryCount=Number(row.category_count||0),selectableCategoryCount=Number(row.selectable_category_count||0),blockers=[];
        if(!row.published_product_id){if(row.validation_status!=='valid')blockers.push('结构未通过');if(row.review_status!=='approved')blockers.push('未审核通过');if(!selectableCategoryCount)blockers.push(categoryCount?'分类只到大类':'未设置标准分类');if(row.source_status!=='active')blockers.push('来源未启用');}
        return { ...row, id: Number(row.id), job_id: Number(row.job_id), source_id: Number(row.source_id),category_count:categoryCount,
          selectable_category_count:selectableCategoryCount,category_ids:String(row.category_ids||'').split(',').map(Number).filter(item=>Number.isSafeInteger(item)&&item>0),category_names:String(row.category_names||'').split('、').filter(Boolean),...summary,publish_ready:!row.published_product_id&&!blockers.length,publish_blockers:blockers,
