...
(function () {
  const labels = { direct: 'V2 直接成功', repaired: 'V2 修复后成功', failed: '生成失败', fallback: '退回 V1', legacy: '旧 V1 任务', pending: '排队 / 生成中', unknown: '结果未知' };
  ...
  function mount({ root, request }) {
    ...
    function compare() {
      ...
      target.innerHTML = `<table><thead><tr><th>指标</th><th>更新前</th><th>更新后</th></tr></thead><tbody>
        ...
        <tr><td>失败 / 回退 V1</td><td>${before.counts.failed} / ${before.counts.fallback}</td><td>${after.counts.failed} / ${after.counts.fallback}</td></tr></tbody></table>
    ...
    async function detail(id) {
      ...
      try {
        ...
        const validations = d.events.filter(e => e.event === 'validation'), first = validations[0], last = validations[validations.length - 1];
        const errorCount = e => e?.issues?.filter(i => i.severity === 'error').length ?? 0;
        target.innerHTML = `<h3>${esc(d.task.title)}</h3><p class="ppt-task-id">${esc(d.task.id)}</p><p><span class="ppt-badge ${esc(d.task.outcome)}">${esc(labels[d.task.outcome])}</span> 初次校验 ${errorCount(first)} 项错误 → 最后校验 ${errorCount(last)} 项错误；模型调用 ${d.task.model_requests ?? '未记录'} 次。</p><p class="ppt-note">${esc(d.task.error || d.diagnostics.fallback_reason?.message || d.diagnostics.v2_failure?.message || '')}</p><div class="toolbar"><button class="primary-btn" data-action="export">导出完整诊断 JSON</button></div><h4>生成与修复过程</h4><ol class="ppt-timeline">${d.events.map(e => {
          const names = { request: '模型请求', response: '模型返回', validation: '排版校验', render_validation: '渲染校验', repair_actions: '执行修复', primary_generation_error: 'V2 失败', fallback_result: 'V1 回退结果', task_failed: '任务失败', fallback_blocked: '已阻止 V1 回退', render_environment_preflight: '环境检查', model_request_reserved: '分配模型调用额度', repair_reserved: '开始修复', fallback_reserved: '开始回退 V1' };
