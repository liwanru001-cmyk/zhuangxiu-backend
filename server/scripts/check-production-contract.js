...
function validateProductionContract(env = process.env, runtime = {}) {
  ...
  try { adminAuthentication.assertConfiguration(env); }
  ...
  if (!chromePath) errors.push('INGESTION_CHROME_PATH must resolve to an installed Chromium executable');

  const fcMatch = (runtime.executable || executable)(env.PRESENTATION_FC_MATCH || 'fc-match', env);
  if (!fcMatch) errors.push('PRESENTATION_FC_MATCH must resolve to an executable');
  const font = String(env.PRESENTATION_V2_FALLBACK_FONT || '').trim();
  if (!font) errors.push('PRESENTATION_V2_FALLBACK_FONT is required');
  else if (fcMatch && runtime.checkFont !== false) {
    try {
...
async function checkConnectivity(contract, env = process.env) {
  ...
  try {
    ...
    return { redis: 'ok', chromium: 'ok', storage: contract.storage_driver === 'oss' ? 'ok' : 'local' };
  } finally {
