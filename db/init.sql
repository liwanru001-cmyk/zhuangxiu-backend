...
CREATE TABLE IF NOT EXISTS project_members (
    ...
    user_id BIGINT UNSIGNED NOT NULL,
    role ENUM('owner', 'owner_member', 'designer', 'merchant', 'project_manager', 'project_supervisor') NOT NULL,
...
CREATE TABLE IF NOT EXISTS project_member_requests (
    ...
    member_role ENUM('owner_member', 'designer', 'project_manager', 'project_supervisor', 'merchant') NOT NULL,
...
-- 项目进度变更申请：非业主新建、修改或删除事项时等待主业主确认
CREATE TABLE IF NOT EXISTS project_progress_change_requests (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    INDEX idx_progress_change_project_status (project_id, status, created_at),
    INDEX idx_progress_change_submitter (submitted_by, status, created_at),
    INDEX idx_progress_change_target (project_id, entity_type, target_id, status),
    FOREIGN KEY (project_id) REFERENCES renovation_projects(id) ON DELETE CASCADE,
